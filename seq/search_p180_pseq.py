...
class SearchP180PSEQ(blankSeq.MRIBLANKSEQ):
    def __init__(self):
        super(SearchP180PSEQ, self).__init__()
        ...
        self.addParameter(key='seqName', string='search_p180', val='fid')
        self.addParameter(key='nScans', string='Number of scans', val=1, field='SEQ')
        self.addParameter(key='larmorFreq', string='Larmor frequency (MHz)', val=10.35397, units=units.MHz, field='RF')
        # Flip angle must be 90 degree though p180 is searched. 
        self.addParameter(key='rfExFA', string='Excitation flip angle (deg)', val=90, field='RF')
        
        self.addParameter(key='rfExAmpSearchRange', string='RF Amp Searching Range (uT)', val=[100,2,200], field='RF',
                          tip="[begin, step, end]")
