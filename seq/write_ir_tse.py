...
def main(plot: bool, write_seq: bool, seq_filename: str = "tse_pypulseq.seq"):
    ...
    TE = 23e-3  # Echo time
    TR = 5700e-3  # Repetition time
    TI = 1400e-3 
    TE_eff = 92e-3
    dwell_set = 31.25e-6 # adc dw time
    dwell = np.round(np.array(dwell_set) * 122.88e6) / 122.88e6
    sampling_time = dwell * Nx # Duration of flat area of readout gradient (sec)
    print(f'dwell time: {dwell}, readout time: {sampling_time}')

    # sampling_time = 6.5e-3
    ...
    t_inv = 3.5e-3
    t_invwd = t_inv + system.rf_ringdown_time + system.rf_dead_time
    fsp_r = 1
    fsp_s = 3
    ...
    shift_steps = np.ceil(Nx / 2 + (TE_eff / TE - 1) * n_echo) 
    ...
    for k_ex in range(n_ex + 1):
        for s in range(n_slices):
            ...
            seq.add_block(rf_inver)
            seq.add_block(TI_fill)
            seq.add_block(gs1)
...
if __name__ == "__main__":
    main(plot=True, write_seq=True, seq_filename = "flair.seq")
