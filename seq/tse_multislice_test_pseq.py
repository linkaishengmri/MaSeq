...
class TSEMultislicePSEQ(blankSeq.MRIBLANKSEQ):
    def __init__(self):
        ...
        self.addParameter(key='larmorFreq', string='Larmor frequency (MHz)', val=10.35382, units=units.MHz, field='IM')
        self.addParameter(key='rfExFA', string='Excitation flip angle (deg)', val=90, field='RF')
        self.addParameter(key='rfReFA', string='Refocusing flip angle (deg)', val=180, field='RF')
        self.addParameter(key='rfSincExTime', string='RF sinc excitation time (ms)', val=0.9, units=units.ms, field='RF')
        self.addParameter(key='rfSincReTime', string='RF sinc refocusing time (ms)', val=.9, units=units.ms, field='RF')
        ...
        self.addParameter(key='etl', string='Echo train length', val=64, field='SEQ')
        self.addParameter(key='effEchoTime', string='Effective echo time (ms)', val=32*16.0, units=units.ms, field='SEQ')
    ...
    def sequenceRun(self, plotSeq=0, demo=False, standalone=False):
        ...
        self.flo_interpreter = PseqInterpreter(
            tx_warmup=100,  # Transmit chain warm-up time (us)
        ...
        self.system = pp.Opts(
            ...
            max_grad=45,  # Maximum gradient strength (mT/m)
        ...
        rf_ex_rect = pp.make_block_pulse(
            flip_angle=self.rfExFA * np.pi / 180,
            duration=t_ex,
            system=self.system,
            phase_offset=np.pi / 2,
        ...
        rf_ref_rect = pp.make_block_pulse(
        ...
        def createBatches():
            ...
            for k_ex in range(1, n_ex + 1):
                ...
                for s in range(n_slices):
                    ...
                    # )
                    rf_ex_rect.phase_offset = rf_ex.phase_offset 
                    rf_ref_rect.phase_offset = rf_ref.phase_offset 
                    
                    batches[batch_num].add_block(gs1)
                    batches[batch_num].add_block(gs2, rf_ex_rect)
                    ...
                    for k_echo in range(n_echo):
                        ...
                        batches[batch_num].add_block(gs4, rf_ref_rect)
            ...
            if plotSeq:
                ...
                batches[batch_num].plot(show_blocks=True, time_range=(0,0.06))
    ...
    def sequenceAnalysis(self, mode=None):
        if False:
            ...
            # [TODO]: Add Rx phase here
            if False:
                rawdata = self.mapVals['data_full']
            ...
            nPoints = self.mapVals['nScans'] * self.mapVals['nPoints'][0] * self.mapVals['etl']
            deadTime = 0 #self.mapVals['deadTime']*1e-3 # ms
            rfRectExTime = self.mapVals['rfSincExTime']*1e-3 # ms
