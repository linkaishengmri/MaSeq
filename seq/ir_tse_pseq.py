...
@Summary: Inversion recovery(IR)-TSE sequence, implemented with PyPulseq and compatible with MaSeq.
"""
...
class IRTSEPSEQ(blankSeq.MRIBLANKSEQ):
    def __init__(self):
        super(IRTSEPSEQ, self).__init__()
        ...
        self.addParameter(key='rfSincInvTime', string='RF sinc inversion time (ms)', val=3.0, units=units.ms, field='RF')
        
        self.addParameter(key='repetitionTime', string='Repetition time (ms)', val=5000.0, units=units.ms, field='SEQ')
        self.addParameter(key='inversionTime', string='Inversion time (ms)', val=1000.0, units=units.ms, field='SEQ')
    ...
    def sequenceRun(self, plotSeq=0, demo=False, standalone=False):
        ...
        TI = self.inversionTime
        rf_inv_flip = self.rfInvFA
        t_inv = self.rfSincInvTime
        ...
        fsp_r = self.fsp_r
        fsp_s = self.fsp_s
        t_invwd = t_inv + self.system.rf_ringdown_time + self.system.rf_dead_time

        # Inversion pulse
        rf_inver = pp.make_sinc_pulse(
            flip_angle=rf_inv_flip * np.pi / 180,
            duration=t_inv,
            apodization=0.5,
            time_bw_product=4,
            use="inversion",
        ...
        TR_fill = (TR - TI - n_slices * TE_train) / n_slices
        ...
        def createBatches():
            ...
            for k_ex in range(n_ex + 1):
                ...
                for s in range(n_slices):
                    ...
                    # )
                    batches[batch_num].add_block(rf_inver)
                    batches[batch_num].add_block(delay_TI)
