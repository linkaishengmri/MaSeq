...
class SearchP90SINCPSEQ(blankSeq.MRIBLANKSEQ):
    def __init__(self):
        super(SearchP90SINCPSEQ, self).__init__()
        ...
        self.addParameter(key='rfExAmpSearchRange', 
                          string='RF Amp Searching Range (uT)', val=[21,1,30], field='RF',
                          tip="[begin, step, end]")
    ...
    def sequenceRunAndAnalysis(self, plotSeq=0, demo=False, standalone=False):
        ...
        recovery_time = self.repetitionTime - 0.5*self.rfSincExTime - self.deadTime - sampling_period * 1e-6
